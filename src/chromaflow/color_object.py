...
from typing import Tuple, cast, Literal, overload
import numpy as np

from .core.spaces import get_space
from .exceptions import ConversionPathError, GamutError
from . import utils
...
class Color:
    ...
    def to(self, target_space: str) -> Color:
        ...
        if full_path is None:
            raise ConversionPathError(f"No conversion path found from '{self.space}' to '{target_space}'.")
        ...
        return Color(space=target_space, values=tuple(cast(Tuple[float, ...], final_values_flat.tolist())))

    # --- Gamut Methods ---
    def in_gamut(self, target_gamut_space: str = "srgb", tolerance: float = 1e-5) -> bool:
        """Checks if the color is within the specified RGB gamut."""
        return gamut.in_gamut(self, target_gamut_space, tolerance)

    @overload
    def to_gamut(self, target_gamut_space: str = "srgb", method: Literal["clip"] = "clip") -> Color: ...
    @overload
    def to_gamut(self, target_gamut_space: str = "srgb", method: Literal["oklch-chroma"] = "oklch-chroma") -> Color: ...
        
    def to_gamut(self, target_gamut_space: str = "srgb", method: str = "oklch-chroma") -> Color:
        """
    ...
    def delta_e(self, other: Color, method: Literal["1976", "2000", "cmc", "jz"] = "2000") -> float:
        """
