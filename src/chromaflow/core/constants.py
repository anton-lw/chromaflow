...
def _derive_xyz_matrix(
    primaries_xy: FloatArray,
    whitepoint_xy: FloatArray,
) -> FloatArray:
    """
    ...
    xw, yw = whitepoint_xy
    xyz_w = np.array([xw / yw, 1.0, (1 - xw - yw) / yw], dtype=np.float64)
    ...
    scalars = np.linalg.inv(matrix) @ xyz_w
    sr, sg, sb = scalars

    # Scale the matrix M by the scalars
    xyz_matrix = matrix * np.array([sr, sg, sb], dtype=np.float64)
...
JZAZBZ_C2 = 2413 / 128
JZAZBZ_C3 = 2392 / 128
JZAZBZ_M1 = 2610 / 16384
JZAZBZ_M2 = 1.7 * 2523 / 32
