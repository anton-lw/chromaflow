...
def delta_e_cmc(
    ...
    T = (
        0.56 + abs(0.2 * np.cos(np.radians(h1 + 168)))
        if 164 <= h1 <= 345
        else 0.36 + abs(0.4 * np.cos(np.radians(h1 + 35)))
    )
    F = np.sqrt(C1**4 / (C1**4 + 1900))
    S_H = S_C * (F * T + 1 - F)
    term1 = delta_L / (lightness_weight * S_L)
    term2 = delta_C / (chroma_weight * S_C)
    term3 = delta_H / S_H
    return float(np.sqrt(term1**2 + term2**2 + term3**2))


def delta_e_2000(
    color1: Color, color2: Color, Kl: float = 1, Kc: float = 1, Kh: float = 1
) -> float:
    ...
    h1_prime, h2_prime = (
        np.degrees(np.arctan2(b1, a1_prime)) % 360,
        np.degrees(np.arctan2(b2, a2_prime)) % 360,
    )
    ...
    S_L, S_C, S_H = (
        1 + (0.015 * (L_bar_prime - 50) ** 2) / np.sqrt(20 + (L_bar_prime - 50) ** 2),
        1 + 0.045 * C_bar_prime,
        1 + 0.015 * C_bar_prime * T,
    )
    R_T = -R_C * np.sin(np.radians(2 * delta_theta))
    term1, term2, term3 = (
        delta_L_prime / (Kl * S_L),
